"""Transit-window analysis of a real TESS SPOC light curve for WASP-19 b.
...
DATA_FILE = DATA_DIR / "tess2019058134432-s0009-0000000035516889-0139-s_lc.fits"
PLANET = "WASP-19 b"
PERIOD_DAYS = 0.78883852
EPOCH_BJD = 2456402.7128
DURATION_HOURS = 1.607
SECTOR = 9
FIGURE_FILE = FIG_DIR / "wasp19b_tess_transit.png"
...
def main() -> dict[str, float]:
    ...
    ax.scatter(hours, (result["flux"] - 1) * 1e6, s=5, alpha=0.12, color="#1971c2", label="quality-filtered cadences")
    ...
    ax.plot(hours[order], (result["model"][order] - 1) * 1e6,
            color="#1971c2", linewidth=2.2, label="fitted box model")
    ax.axvspan(-DURATION_HOURS / 2.0, DURATION_HOURS / 2.0, color="#1971c2", alpha=0.08)
