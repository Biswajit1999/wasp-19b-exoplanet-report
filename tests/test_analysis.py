...
def test_pipeline_reproduces_committed_statistics():
    analysis.main()
    values = read_summary()
    assert np.isclose(values["depth_ppm"], 16844.2223231, rtol=0, atol=0.1)
    assert np.isclose(values["chi_square_flat"], 52678.9969405, rtol=1e-9)
    assert np.isclose(values["chi_square_box"], 17807.0797124, rtol=1e-9)
    assert np.isclose(values["delta_chi_square"], 34871.9172281, rtol=1e-9)
